...
def run_prediction(j,paragrapha):
  ...
  for session in paragrapha:
        ...
        if count>(j-1)*2000 and count<=j*2000:

             paragrapha2[session]= paragrapha[session]
        else:
             break
...
#转换成id
  ...
  for session in paragrapha2:
      ...
      # print("page count:"+str(count))
       ...
       for key in paragrapha2[session]:
       #      print(key)
             s=key[2]
    #print(s)
             u_id=key[0]
...
#  f = open(output_file_path+"/result.txt", 'w')
  filename="./results5/result_"+str(f_tag)+".txt"
...
if __name__ == '__main__':
  ...
  #tag="result0"
      ...
      page=math.ceil(ii/2000)
      print("page:",page)
      #input()
      for j in range(int(page)):
              run_prediction(j,paragrapha)
      print("end")
