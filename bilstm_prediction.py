...
with tf.variable_scope('outputs'):
     ...
     y_predict = tf.cast(tf.argmax(y2, axis=1), tf.float32)
...
#对输出标签 定义为1行多列
y_label_reshape = tf.cast(tf.reshape(y_label, [-1]), tf.float32)
...
cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=y_predict, labels=y_label_reshape))
...
for j in range(10000):
  for i in range(0,500,5):
   ...
   # mf=tf.expand_dims(x[i:i+5],0)

  #  mf=tf.expand_dims(mf,0)
    mf=tf.stack(list_a, axis=0)  
    print(mf)
    list_b=[]
    for li in y_t[i:i+10]:
       list_b.append(li)
    yl=tf.expand_dims(list_b,2)
 
    x_feed=sess.run(mf)

    y_feed=sess.run(tf.squeeze(yl))
    print(y_feed)
    _,acc,loss=sess.run([train,accuracy,cross_entropy] ,feed_dict={x_input:x_feed,y_label:y_feed})
    print("acc:",acc)
    print("loss:",loss)
