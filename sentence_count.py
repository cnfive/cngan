...
def  run_prediction():
  ...
  for session in paragrapha:
         ...
         for key in paragrapha[session]:
              u_tag=key[1]
              if key[2] not in sentence_dic:
                    
                    num_sessionid_list=[[session,num,u_tag]]

                    sentence_dic[key[2]]=[1,num_sessionid_list]
              else:
                    s_count=sentence_dic[key[2]][0]+1

                    num_sessionid_list=sentence_dic[key[2]][1]

                    num_sessionid_list.append([session,num,u_tag])

                    sentence_dic[key[2]]=[s_count,num_sessionid_list]
              num=num+1
  ...
  for sentence in sentence_dic:
      ...
      for txt in sentence_dic[sentence][1]:
         ...
         f.writelines(txt[0]+"        "+str(txt[1])+"        "+txt[2]+"        "+sentence+"        "+str((sentence_dic[sentence])[0])+"\n")
